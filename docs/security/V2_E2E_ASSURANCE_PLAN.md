...
| 7 | V2 Golden Path | IN_PROGRESS | IA-093: the current Chromium lifecycle has strong canonical write/reload/Join coverage, but its initial security bootstrap remains test-owned: the harness directly constructs ManifestV6/genesis WriterGrant/RecoveryArtifactV6 and directly initializes StateV6, WriterDeviceKeyV2 and RootWrapV6 before productive native selection. GATE-E2E-01 therefore remains open until new-diary/V2 bootstrap crosses the productive application/service boundary without a test-only security bootstrap. |
...
| 10 | Forced Takeover / Pending-Rekey | IN_PROGRESS | The Chromium case proves productive Forced Takeover/fencing and the L2 productive integration suite covers Pending-Rekey/maintenance/Phase-B semantics. The §12/GATE-E2E-03 browser lifecycle, however, does not yet demonstrate the required visible durable Pending-Rekey maintenance phase and its normal-write fence before Phase-B completion; current browser summary reaches `maintenanceOnly: false`. Keep the L2 evidence, but the L3 gate remains open. |
| 11 | Recovery-Rekey | IN_PROGRESS | Productive L2 integration provides substantial R1→R2, transition, Pending-Rekey, Phase-B and crash-point evidence. §13 additionally requires browser restart → unlock → resume across persisted transition points. The existing closure text explicitly says browser restart/UI is outside its evidence, so GATE-E2E-04 is not yet satisfied. |
...
| 14 | Unknown-Outcome-Matrix | IN_PROGRESS | Productive L2 coordinator/ceremony coverage is substantial and the controlled provider supplies deterministic no-commit, committed-response-lost, duplicate, delay and intervening-append faults. However §27 defines PR-security as the complete simulated **browser E2E** crash/tamper/Unknown-Outcome suite, while the current closure explicitly proves GATE-E2E-07 only at L2. No browser test currently injects these provider outcomes through the productive app/session boundary. Keep the L2 regressions; add L3 coverage before closing the E2E gate. |
...
| 16 | Plaintext-/Credential-Leak-Scanner | IN_PROGRESS | Browser persistence scanning covers DOM, Web Storage, IndexedDB object-store values and Cache Storage for health/OAuth sentinels. §20 additionally requires instrumentable Diary-origin requests, logs and error messages. The current Golden Path scanner does not capture requests/console/errors, and the Auth suite's persistence helper likewise does not establish those surfaces. INV-01 evidence is useful but GATE-E2E-09 remains open until these channels are scanned or explicitly justified as non-instrumentable. |
| 17 | Lock/Passphrase | IN_PROGRESS | Strong L2 RootWrap/Passphrase/WebAuthn and product-flow coverage exists, but §21 explicitly requires Browserneustart plus lock-state enforcement for protected read, write, Handoff and Recovery operations. Current closure cites Chromium UI cases without demonstrating the complete browser lifecycle/security-operation matrix. Preserve the existing regressions; close only after the required L3 lock/reload/operation assertions exist. |
...
`DONE 9/25; IN_PROGRESS 9/25; BLOCKED 4/25; NOT_STARTED 3/25; remaining internally actionable: 12`
