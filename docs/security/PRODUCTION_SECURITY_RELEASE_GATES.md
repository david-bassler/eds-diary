...
| Transferable Single Writer v2 | **ARCHITEKTURRAHMEN DEFINIERT / PROTOKOLL NICHT EINGEFROREN / NICHT IMPLEMENTIERT** | Mehrere Geräte dürfen dasselbe Tagebuch lesen, aber nur eine remote verifizierte Writer-Key-Authority darf Fachcommits erzeugen. Vor Implementierung müssen exaktes v2 Wire-/Schema-/Signaturprofil, Writer-Key-Speicher-/Anti-Cloning-Threat-Boundary, manifestgebundener Gen‑1-Trust-Root, Recovery-Takeover-Proof, v1→v2-Migration, Join/Handoff/Fencing sowie Rotation-/Takeover- und Live-Google-Konkurrenztests abgeschlossen werden. Ein generischer Browser-`CryptoKey` allein beweist nicht die Einzigartigkeit eines physischen Geräts. |
...
`SECURITY/SPEC DECISION REQUIRED: freeze exact transferable-single-writer-v2 wire, signature and recovery-takeover protocol before code`
