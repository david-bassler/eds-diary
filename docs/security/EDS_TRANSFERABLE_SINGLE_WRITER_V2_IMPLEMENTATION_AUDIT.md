...
| IA-031 | V2-03↔V2-04 / fresh domain-write reconciliation | `V2DomainWritePreparer.prepareAndPersist()` performs its mandatory fresh canonical verify and then persists the reconciled StateV6 through `replaceState()` only. It does not run the same verified-disposition commit used by the Coordinator pull. Therefore a locally prepared envelope whose Writer authority has just become stale can remain `prepared/pending` instead of being atomically moved to `stale_writer_pending`; IA-023 is closed on the Coordinator path but can be bypassed by the domain-write prepare path. | **OPEN.** Fresh prepare reconciliation must share the semantic disposition/quarantine commit (or an equivalent single primitive) before deciding whether another domain write is allowed. Add a regression where an old pending envelope exists, remote Writer advances, and the next prepare attempt both fails read-only and quarantines the old envelope. |
| IA-032 | V2-04 / verified capability branding | `VerifiedRecoveryTakeoverStagingV2` and `VerifiedPersistedRecoveryArtifactV6` are exported classes with public constructors that register themselves in their respective WeakSet brands. Any internal caller can therefore manufacture the “verified” capability without performing the mandated AEAD/keypair verification or persistent/readback verification. For staging this can permit remote epoch creation without proved possession of the matching takeover private key; for artifacts it defeats the one-shot persistence precondition before publish. | **OPEN.** Make these capabilities unforgeable outside their verification/persistence factories (private/module-secret construction) and add negative architecture/type/runtime tests proving direct construction cannot satisfy the provider gates. |
| IA-033 | V2-02↔V2-04 / Manifest trust-root provenance | `manifestTrustRootV6(cells,payload)` accepts ManifestV6 public cells and an independently supplied protected payload, validates each structurally, then combines the payload authority with the fingerprint of the cells. It does not itself prove that `payload` was obtained by decrypting those exact cells. Because `VerifiedManifestTrustRootV2` is only a structural interface, a future production caller can mint a verifier trust root from unrelated but individually valid inputs and bypass the V2-02 trust-root assumption. Current reviewed callers happen to call `openManifestV6` first, but the exported security boundary is not fail-closed. | **OPEN.** Collapse decrypt+validate+trust-root creation into one provenance-preserving API or return a non-forgeable opened-manifest capability that `manifestTrustRootV6` requires. Pin a negative vector using valid cells plus a different valid payload. |
| IA-034 | V2-03↔V2-04 / FreshCanonicalV2Source wiring | V2-03 deliberately left `FreshCanonicalV2Source.verifyNow()` for V2-04 to implement as a new provider read plus `canonical_full`. PR #53 adds the v2 transport, codec and provider session, but contains no production `FreshCanonicalV2Source` implementation/factory and no `verifyNow` wiring. The primitive pieces exist, but the promised fresh-read security seam remains unimplemented. | **OPEN / scope-completion gap.** Add one provider-bound fresh source that reads the bound remote resource and calls the v2 codec’s canonical-full verifier on every invocation; it must not cache a snapshot or accept caller-supplied verified state. |
