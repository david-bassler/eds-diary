...
| IA-021 | V2-03 / RootWrapV6 completeness | The initial V2-03 cut treated root-unlock as an external dependency and had not yet provided the versioned v6 wrap primitives/store even though StateV6 §18.1 explicitly freezes new v6 passphrase/PRF domains and forbids falling back to RootWrapV5. | **Fixed.** V2-03 now has closed RootWrapV6 types/validation, best-effort/passphrase/PRF v6 crypto, and separate v2 wrap/wrapping-key persistence; v1 RootWrapV5 remains unchanged. |
| IA-022 | V2-03 / immutable envelope persistence | The first v2 local store accepted the caller-supplied `PreparedEnvelope.bytesHash` without reproducing it and relied on astronomically unlikely CSPRNG non-collision rather than explicitly rejecting IV reuse across different local envelope IDs. A corrupt/self-conflicting local row would later be rejected by the remote verifier, but should not be persistable locally. | **Fixed.** Persistence recomputes the exact JCS row hash, rejects mismatches, checks IV ownership before commit, and backs that invariant with a unique IndexedDB IV index. |
| IA-023 | V2-03 / generic-vs-profile verified state | The first v2 WriteAuthority adapter validated the canonical profile state but did not require the shared `VerifiedRemoteState` accepted/stale/verified sets and `retired` flag to be byte-semantically identical to the embedded canonical v2 result. The shared Coordinator consumes those generic fields, so divergence would split durability/retirement semantics between layers. | **Fixed.** The adapter now requires exact set equality for verified/accepted/stale envelope IDs and `retired === source_epoch_sealed` before any local authority decision. |
| IA-024 | V2-03 / PRF enrollment gate | The initial RootWrapV6 PRF API accepted valid-shaped PRF output without an explicit typed assertion that post-enrollment PRF verification had succeeded. §18.1 requires post-enrollment verification and user verification before the PRF wrap is created. | **Fixed at this layer.** RootWrapV6 creation now requires `verified:true` in its enrollment material and rejects unverified material. The later WebAuthn product integration remains responsible for producing that marker only after the required credential/user-verification flow. |
| IA-025 | V2-03 / local generation race | The first envelope/quarantine store paths checked `operation_generation` before opening the final readwrite transaction. The current domain service holds a diary Web Lock, but future shared Coordinator paths must not depend on that incidental caller ordering for state CAS. | **Fixed.** Both immutable-envelope commit and stale-writer quarantine re-read and compare StateV6 generation inside the committing IndexedDB transaction before mutating state/outbox/reservation rows. |
| IA-026 | V2-03 / restore-state overconstraint | The initial StateV6 validator required `remote_anchor`, live `remote_binding` and the verified-writer cache to appear together. That is too strong for legitimate `local_offline`/`offline_restored` backup or recovery states, which may retain an authenticated freshness anchor while no live remote binding has yet been re-established. | **Fixed.** The three-way requirement now applies only to `epoch_status="active"`; non-active restore/staging states may retain an anchor without gaining writer authority. A regression vector pins `offline_restored + remote_anchor + no live binding`. |
| IA-027 | V2-03 / IV uniqueness scope | The first dedicated v2 IndexedDB store made the IV index globally unique across all local epochs. The verifier's `iv_reuse_across_envelope_ids` invariant is epoch-local; globally rejecting an otherwise valid IV in an unrelated epoch would add a non-normative availability restriction. | **Fixed.** The local unique index is now the compound key `[epoch_id, iv]`, matching the verifier/protocol scope. |
...
8. **A dedicated v2 local security database is an implementation boundary, not a wire/protocol fork.** V2-03 intentionally avoids upgrading or mutating the existing v1/V5 IndexedDB schema. V2-05 migration will populate the v2 store when a new v2 epoch is created. This keeps the frozen v1 database/recovery assumptions testable and unchanged; later consolidation would require its own explicit migration review.
