...
IA-046 are recorded **OPEN before remediation**. IA-045 is a real protocol
regression introduced by the prior IA-044 hardening; IA-046 is the missing
productive assurance coverage that allowed that regression to remain green.
...
| IA-044 | V2-05 / one-shot cutover physical rows | The productive cutover helpers treated multiple byte-identical physical copies of the prepared v1 Announcement or v2 Confirmation as an acceptable durable one-shot control. Although normal unknown-outcome reconciliation reads back before retrying, a provider anomaly or concurrent duplicate append could leave repeated physical activation rows that later resume paths would collapse semantically. The activation boundary must be stricter than ordinary retry-row semantics. | **Fixed.** V2-05 now requires exactly one physical Announcement row after the frozen v1 Source prefix and exactly one physical Confirmation row after the frozen Successor staging anchor. The same uniqueness check is repeated by the activation verifier on every later resume stage, so duplicates appearing after `announcement_durable` or `confirmation_durable` fail closed before Backup/local switch. Dedicated regressions inject both duplicate cases. |
| IA-045 | V2-05/V2-06 / activation-boundary retry and post-activation suffix semantics | The IA-044 hardening changed the productive profile-upgrade cutover to require exactly one physical Source Announcement and exactly one physical Successor Confirmation. That contradicts the frozen protocol's general byte-identical retry rule and §21 step 20, which explicitly includes immediately following byte-identical Confirmation retries in successor_activation_anchor and permits a fully valid post-activation suffix. The current helper therefore rejects legitimate Unknown-Outcome retries and any valid Fachrow/WriterGrant appended after Confirmation; V2-06 Join also rejects a profile-upgrade Source containing a byte-identical Announcement retry. | **OPEN.** Restore decision-row semantics: the first row after the bound Source/staging prefix must be the prepared one-shot control; immediately following byte-identical retries are semantic no-ops and must be included where specified, while later post-activation Successor rows are canonical_full-validated rather than treated as cutover races. Reconcile IA-040/IA-044 wording with the normative profile. |
| IA-046 | V2-05 / productive assurance coverage | The state-machine harness covers abstract post_activation_superseded outcomes, but the productive profile-upgrade integration suite has no end-to-end vectors for §21/§23 post-activation Fachrow/WriterGrant continuation, RecoveryAuthorityTransition/seal supersession, or Confirmation retry duplicates. Because these productive vectors were absent, IA-045 remained CI-green. | **OPEN.** Add productive integration regressions for allowed Confirmation retries and post-activation Fachrow/WriterGrant suffixes, plus terminal supersession from Recovery-state advance/seal before local switch. |
| IA-047 | V2-05 / split-read activation freshness | `verifyActivationBoundary(verified,result)` performs another fresh Successor read/`canonical_full`, but its caller continues to use the older `verified/result` snapshot for Recovery-supersession and activated-Backup decisions. If the second read has already observed a valid RecoveryAuthorityTransition or a sealing RotationAnnouncement, the service can still create/persist an activated Backup from the older prefix before the later pre-switch verify finally returns `post_activation_superseded`. This does not currently permit the local switch, but it violates the §21 ordering that supersession must be decided before the activated Backup is created. | **OPEN.** Make activation-boundary verification operate on one explicit verified snapshot, or return/use the freshest re-read result atomically in the caller. Add a productive race regression that advances Recovery/seals between the caller read and activation-boundary check and proves no activated Backup is created. |
| IA-048 | V2-05 / historical migration proof vs post-activation graph | `verifyProfileUpgradeMigrationIntegrityV2()` derives Successor provenance from the final `accepted_revision_graph`. A valid post-activation Fachrow or WriterGrant can therefore change current heads after the historical EpochMigrationV2 was already valid, causing the profile-upgrade migration proof to fail retroactively (`migration provenance head-count mismatch`). This contradicts §21 step 20, which explicitly permits a fully valid post-activation suffix after Confirmation. | **OPEN.** Verify migration provenance against the canonical Successor prefix frozen by `successor_staging_anchor` (or an equivalent historical verifier result), not against the final post-activation graph. Add productive Fachrow/WriterGrant suffix regressions proving the historical migration remains valid while current authority/heads continue forward. |
...
- duplicate physical v1 profile-upgrade Announcement or v2 Confirmation rows remain fatal across V2-05 crash-resume stages, including after their durable state transitions;
