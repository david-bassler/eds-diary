...
| IA-081 | V2-10 / UI repository protocol bypass after v2 selection | The app-level pain/activity/medication/settings repositories still call the v1 `localDatabase` CRUD functions directly. After `activeProtocolSelectionV2` exists, those calls can continue creating v1 revisions in the retired/local placeholder path instead of entering `V2DomainWritePreparer`, so UI writes bypass StateV6, fresh canonical verification, Writer authority, v2 signatures and stale-writer fencing. `initializeDataLayer`/`GoogleSyncSettings` are likewise v1-only. | **OPEN.** Add an explicit protocol-selecting application data layer. When v2 is selected, every domain mutation must use a freshly verified `V2DomainWritePreparer`; reads must materialize only the accepted v2 revision graph; v1 CRUD must remain reachable only when no v2 selection exists. V2 authenticated sessions must install a v2 coordinator/synchronizer rather than `SingleWriterSyncService`. Add regressions proving a v2-selected read-only device cannot mutate through existing repositories. |
| IA-082 | V2-10 / verified local v2 read-model persistence | `IndexedDbV2CoordinatorStore.commitVerifiedPull()` reconciles StateV6 and outbox dispositions but does not persist newly verified remote EnvelopeV6 rows or an authenticated accepted-order index. Consequently the app cannot satisfy the architecture rule that reading from already-verified local state remains available offline without either trusting stale/rejected local rows or re-reading Google. | **OPEN.** On every canonical verified pull, atomically import immutable verified remote envelope bytes into the encrypted local journal and persist an HMAC-authenticated read-model index bound to epoch/manifest/remote anchor and the accepted envelope IDs in first physical order. Offline materialization must decrypt only those accepted IDs and never persist health plaintext. Cache/StateV6 anchor mismatch or MAC failure must fail closed. |
| IA-083 | V2-10 / foreign stale-row import into local stale-pending outbox | The first V2-10 verified-pull import treated every remote `stale_writer_envelope_id` that was not already local like an accepted remote row: it persisted the encrypted bytes locally and created a synthetic `stale_writer_pending` outbox entry. `stale_writer_pending` is a local quarantine obligation for this installation's own previously prepared Writer envelope; importing stale rows authored by other devices would manufacture local remediation state, inflate the pending count and make a clean read-only Join appear to own foreign stale work. | **OPEN.** Import only canonically accepted remote envelopes needed for the offline read model. A remote stale row is never synthesized into the local outbox. If the same envelope already exists locally, the existing authenticated outbox entry may transition to `stale_writer_pending` through normal disposition reconciliation. Add a regression with a fresh read-only device pulling a physical stale Writer row from another device. |
| IA-084 | V2-10 / Pending-Rekey replacement-device UI dead end | The first V2-10 Writer settings rendered Forced Takeover only when `writerStatus==='read_only' && !recoveryRekeyRequired`. That hides the one recovery action required after a fresh-device Join into a remotely pending Recovery-Rekey. Exact Protocol/Architecture require `Join -> Forced Takeover (maintenance-only) -> remote_pending_rekey_adoption -> Phase B`; cooperative Handoff and normal domain writes remain blocked, but Forced Takeover must stay available. | **OPEN.** On a read-only Pending-Rekey device, explicitly offer Recovery-key-authorized Forced Takeover and explain that the resulting Writer remains maintenance-only. Once canonical takeover succeeds, surface only the Pending-Rekey completion action until Phase B finishes. Add a UI/data-layer regression for this state sequence. |
| IA-085 | V2-10 / Settings profile-transition listener initialization | The first profile-aware `GoogleSyncSettings` registered `onSyncState()` before initializing its local `cancelled` guard even though `onSyncState()` synchronously invokes the new listener. That creates a temporal-dead-zone runtime failure on mount. The listener also refreshed durability without updating `protocolProfile`, so an in-place v1→v2 upgrade could leave legacy v1 controls visible until remount. | **OPEN.** Initialize cancellation state before listener registration and make every sync-state refresh re-read `remoteSessionStatus`, switching the settings surface to v2 immediately when the active protocol selection changes. Add/retain UI build coverage. |
| IA-086 | V2-10 / local strong-protection UI remained v1-only after v2 selection | `App` and `LocalSecuritySettings` still read/enrolled/unlocked/locked only the v1 `RootWrap`, while the active v2 RK is protected by a separate `RootWrapV6` in the v2 security store. After Join/upgrade, enabling Passphrase or PRF in the visible UI could therefore strengthen only the retired/placeholder v1 context and leave the active v2 RK under its previous best-effort wrap, while the UI claimed strong local protection. On a v1→v2 profile-upgrade device, the inverse is also relevant: historical v1 health envelopes remain locally present, so strengthening only v2 would leave the retired source decryptable under an older weaker local wrap. | **OPEN.** Make the existing local-security API dispatch to the active protocol. For active v2, status/unlock/lock must use the selected RootWrapV6 and authenticate StateV6 before exposing data. Passphrase/PRF enrollment must rewrap the active v2 RK with persistent readback and, when the retained v1 source belongs to the same diary, strengthen that historical source first so a crash cannot yield a UI-visible strong v2 mode while same-diary historical v1 health data remain best-effort. Add regressions for v2 best-effort→passphrase/PRF, lock/unlock, and same-diary retained-source ordering. |
