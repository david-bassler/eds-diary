...
| IA-062 | V2-07 / release-gate inventory drift | After Cooperative Handoff became productive and fully validated, `PRODUCTION_SECURITY_RELEASE_GATES.md` and its internal TODO still described the v2 implementation boundary as V2-01…V2-06 and listed Cooperative Handoff as open. The protocol code was correct, but the authoritative release-status inventory could cause reviewers to reason from an outdated implementation boundary. | **OPEN.** Update the release-gate/status inventory to V2-01…V2-07, mark Cooperative Handoff internally implemented/validated, retain Forced Takeover, native Rotation/Recovery-Rekey, App/UI wiring and Live-Google Parallel-Append as open, and pin the exact current V2-07 reviewed head. |
| IA-063 | V2-07 / stale WriterGrant prefix-advance proof | IA-061 added a persistence-layer stale-evidence gate, but the first cut only required authenticated StateV6 `remote_anchor` to differ from the prepared Grant's `authority_anchor`. A same-height/different-hash anchor is not a proof of physical prefix advancement and must never authorize terminal stale classification, even though the normal canonical reconciliation path would already reject such a fork. | **OPEN.** Require the authenticated StateV6 remote anchor to use the v2 profile and have `covered_row_count` strictly greater than the prepared authority anchor before a terminal WriterGrant `stale` transition. Add a direct-storage regression proving a same-height/different-hash authenticated anchor is insufficient. |
