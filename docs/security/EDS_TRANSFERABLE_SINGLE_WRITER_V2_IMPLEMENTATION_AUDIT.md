...
| IA-057 | V2-07 / stale Handoff control quarantine | When a prepared Handoff Grant becomes terminal `stale`, the operation state transitions correctly but its ceremony-owned outbox entry (`authority=null`) remains `prepared`. The generic Coordinator intentionally ignores such controls, so nothing later reclassifies it. A future BackupV6 would therefore treat the row as pending even though replay classifies it `stale_grant_rejected` / another stale control disposition, causing backup verification to fail instead of preserving it in the stale/quarantine category. | **OPEN.** Before closing V2-07, terminal stale WriterGrant operations must atomically or crash-resumably move their exact local Grant envelope to the existing stale quarantine class; regression must cover both an absent stale Grant after an intervening row and a physically present `stale_grant_rejected` row. |
| IA-058 | V2-07 / Handoff pending-envelope gate | `noPendingDomainEnvelopes()` currently rejects every non-durable domain outbox entry, including terminal `stale_writer_pending` rows. §15 forbids unresolved non-durable Pending-Envelopes from A; quarantined stale rows are already resolved historical material and must not permanently prevent a later cooperative transfer. | **OPEN.** The Handoff gate must distinguish unresolved `prepared/pending` current Writer envelopes from terminal stale quarantine, while retaining fail-closed blocking for every unresolved current-authority domain row. |
