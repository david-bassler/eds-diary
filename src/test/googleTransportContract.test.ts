...
class ContractClient implements GoogleApiClient {
  ...
  async request<T>(url:string,init?:RequestInit):Promise<T>{
    ...
    if(parsed.pathname==='/drive/v3/files'){const q=decodeURIComponent(parsed.searchParams.get('q')??'');if(q.includes("appProperties has"))return {files:[{id:'file-1',name:'renamed-by-user',mimeType:'application/vnd.google-apps.spreadsheet',trashed:false,appProperties:{app_format:'sync-v5',epoch_locator:await epochLocator(diaryId,epochId)}}]} as T;return {files:[]} as T}
...
describe('Google single-writer provider contract',()=>{
  ...
  it('discovers an epoch by its bound Drive property even if the file was renamed',async()=>{const client=new ContractClient(),adapter=await transport(client),found=await adapter.discover(await epochLocator(diaryId,epochId));expect(found).toEqual([{remoteId:'file-1',locator:await epochLocator(diaryId,epochId)}])})
