...
class ContractClient implements GoogleApiClient {
  ...
  async request<T>(url:string,init?:RequestInit):Promise<T>{
    ...
    if(url.includes('/drive/v3/about?'))return {user:{permissionId:owner}} as T
    if(url.includes('/drive/v3/files/file-1/permissions?'))return {permissions:[{id:owner,type:'user',role:'owner',deleted:false}]} as T
    if(url.includes('/drive/v3/files/file-1?fields=id,mimeType'))return {id:'file-1',mimeType:'application/vnd.google-apps.spreadsheet',trashed:false,ownedByMe:true,shared:false,isAppAuthorized:true,appProperties:{app_format:'sync-v5',epoch_locator:await epochLocator(diaryId,epochId)}} as T
    if(url.includes('sheets.googleapis.com/v4/spreadsheets/file-1?fields=')&&!url.includes('ranges='))return {sheets:[{properties:{sheetId:1,title:'_m',sheetType:'GRID',gridProperties:{rowCount:1,columnCount:4}},merges:[]},{properties:{sheetId:2,title:'_r',sheetType:'GRID',gridProperties:{rowCount:1,columnCount:3}},merges:[]}]} as T
    if(url.includes("ranges='%5Fm'%21A1%3AD1")||url.includes("ranges='_m'!A1:D1"))return {sheets:[{properties:{sheetId:1,title:'_m'},data:[{startRow:0,rowData:[{values:manifest.map(value=>({userEnteredValue:{stringValue:value}}))}]}]}]} as T
    if(url.includes("ranges='%5Fr'%21A1%3AC1")||url.includes("ranges='_r'!A1:C1"))return {sheets:[{properties:{sheetId:2,title:'_r'},data:[]}]} as T
