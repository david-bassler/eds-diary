...
describe('create reconciliation',()=>{it('binds only one authenticated candidate and stops ambiguity',async()=>{const t=new InMemoryTransport(),c=new GoogleSheetsSingleWriterProfileCodec(async () => undefined),manifest=['sync-v5','5','iv','ct'];t.remotes.set('loc',{manifest,rows:[]});const fingerprint=await c.authenticateManifest({manifest,rows:[]});const base={locator:'loc',manifestFingerprint:fingerprint,status:'creation_pending' as const,remoteId:null};expect((await createOrReconcile(base,manifest,t,c)).status).toBe('bound');t.remotes.set('loc-copy',{manifest,rows:[]});expect((await reconcileCreation(base,t,c)).status).toBe('ambiguous')})})
