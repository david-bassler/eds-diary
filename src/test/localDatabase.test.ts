...
it('removes a reappeared legacy localStorage value from sealed v10 without inventing v11',async()=>{
  const db=await import('../data/localDatabase'),prior=Object.getOwnPropertyDescriptor(globalThis,'localStorage'),values=new Map<string,string>(),storage={getItem:(key:string)=>values.get(key)??null,setItem:(key:string,value:string)=>{values.set(key,value)},removeItem:(key:string)=>{values.delete(key)},clear:()=>values.clear()}
  Object.defineProperty(globalThis,'localStorage',{value:storage,configurable:true})
  try{
    await db.__localDatabaseTesting.resetForTesting()
    const before=await db.__localDatabaseTesting.openDatabase();expect(before.version).toBe(10)
    storage.setItem('eds-diary-activity-types-v1','["legacy"]')
    await db.__localDatabaseTesting.resetForTesting()
    await expect(db.getAllRecords(db.LOCAL_STORES.painEntries)).resolves.toBeTruthy()
    expect(storage.getItem('eds-diary-activity-types-v1')).toBeNull()
    const after=await db.__localDatabaseTesting.openDatabase();expect(after.version).toBe(10)
  }finally{
    await db.__localDatabaseTesting.resetForTesting()
    if(prior)Object.defineProperty(globalThis,'localStorage',prior);else delete (globalThis as {localStorage?:unknown}).localStorage
  }
})
