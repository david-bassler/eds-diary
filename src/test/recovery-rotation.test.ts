...
describe('recovery continuity', () => {
  it('activates only after authenticated discovery and a complete positive bootstrap verification',async()=>{
    ...
    const artifact=await createRecovery({diary_id:diary,epoch_id:epoch,key_id:keyId,RK_epoch:base64Url(root),manifest_fingerprint:fingerprint,remote_anchor:await createAnchor(diary,epoch,rows),google_account_binding:account,recovery_generation:generation,created_at:'2026-09-16T10:01:00.000Z'},urs),candidate=await recoverRootKeyCandidate(artifact,urs),authority=await session.recoveryAuthority(transport,'recovery-locator','remote-positive'),verifier=new RecoveryBootstrapVerifier({authority,schemas:DOMAIN_SCHEMA_REGISTRY});let persisted=false
    await activateRecoveredRoot(candidate,verifier,async()=>{persisted=true;return{rootWrapReadback:true,stateMacVerified:true,envelopeJournalVerified:true}});expect(persisted).toBe(true)
  })
  ...
  it('rejects a cryptographically valid backup whose epoch is already retired',async()=>{
    const diary=b(61,16),epoch=b(62,16),successor=b(63,16),keyId=b(64,16),root=randomBytes(32),urs=randomBytes(32),generation=3,account=b(65,32),commitment=await recoveryCommitment(urs,new Uint8Array(16).fill(61),generation),salt=await deriveEpochSalt(new Uint8Array(16).fill(61),new Uint8Array(16).fill(62)),protectedManifest={diary_id:diary,epoch_id:epoch,key_id:keyId,recovery_generation:generation,recovery_urs_commitment:commitment,diary_marker:'epoch-manifest-v5' as const,crypto_suite:'A256GCM-HKDF-SHA256-v5' as const,sync_profile:'google-sheets-single-writer-v1' as const,created_at:'2026-09-16T11:30:00.000Z',google_account_binding:account,predecessor_epochs:[],record_schema_allowlist:[...SCHEMA_ALLOWLIST],record_schema_registry_hash:await schemaRegistryHash(DOMAIN_SCHEMA_REGISTRY),protocol_limits:{max_payload_bytes:16380 as const,padding_buckets:[1024,2048,4096,8192,16384],max_unique_envelopes:100000 as const,max_unique_canonical_bytes:134217728 as const,max_remote_physical_rows:100000 as const,max_remote_physical_canonical_bytes:134217728 as const,max_canonical_row_bytes:21936 as const}},prepared=await prepareManifest(root,salt,{diaryId:diary,epochId:epoch},protectedManifest),fingerprint=await manifestFingerprint(prepared),manifest=[prepared.format,prepared.version,prepared.manifestIv,prepared.manifestCiphertext] as const
    const announcement:Revision={record_type:'rotation_announcement',record_schema:'rotation-announcement-sw-v1',record_id:b(66,16),revision_id:b(67,32),parent_revision_ids:[],record_status:'control',record_data:{rotation_id:b(68,32),from_epoch_id:epoch,successor_epoch_id:successor,successor_creation_locator:b(69,16),successor_manifest_fingerprint:b(70,32),rotation_kind:'normal'},migration_origin:null,protocol_created_at:'2026-09-16T11:31:00.000Z'},envelope=await prepareEnvelope(root,salt,{diaryId:diary,epochId:epoch},announcement,{reserve:async()=>undefined,persist:async()=>undefined}),rows=[envelopeRow(envelope)] as const,remoteAnchor=await createAnchor(diary,epoch,rows),artifact=await createRecovery({diary_id:diary,epoch_id:epoch,key_id:keyId,RK_epoch:base64Url(root),manifest_fingerprint:fingerprint,remote_anchor:remoteAnchor,google_account_binding:account,recovery_generation:generation,created_at:'2026-09-16T11:32:00.000Z'},urs),backup=await createBackup({rootKey:root,epochSalt:salt,diaryId:diary,epochId:epoch,keyId,manifestFingerprint:fingerprint,epochManifestPublic:manifest,remoteRows:rows,localEnvelopes:[envelope],remoteBound:true,createdAt:'2026-09-16T11:33:00.000Z'}),candidate=await recoverRootKeyCandidate(artifact,urs),authority=await IndependentBootstrapAuthority.fromIndependentBackup(backup),verifier=new RecoveryBootstrapVerifier({authority,schemas:DOMAIN_SCHEMA_REGISTRY})
    await expect(activateRecoveredRoot(candidate,verifier,async()=>({rootWrapReadback:true,stateMacVerified:true,envelopeJournalVerified:true}))).rejects.toThrow(/retired epoch/)
  })
  ...
  it('rejects self-confirmation through a caller-assembled epoch verifier', async()=>{
    ...
    await expect(activateRecoveredRoot(candidate,forged as unknown as RecoveryBootstrapVerifier,async()=>({rootWrapReadback:true,stateMacVerified:true,envelopeJournalVerified:true}))).rejects.toThrow('independently authenticated')
    ...
    await expect(IndependentBootstrapAuthority.fromAuthenticatedRemoteDiscovery(transport,'locator','locator-resource','')).rejects.toThrow('authenticated single-writer provider session')
  })
