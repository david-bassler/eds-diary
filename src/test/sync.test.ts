...
function setup(envelope: PreparedEnvelope) { let anchor: RemoteAnchor|null=null; let durable=''; const transport=new InMemoryTransport(); transport.remotes.set('remote',{manifest:['sync-v5','5','iv','ciphertext'],rows:[]}); const store:CoordinatorStore={readAnchor:async()=>anchor,pending:async()=>[envelope],generation:async()=>1,commitDurable:async(id,next,generation)=>{expect(generation).toBe(1);durable=id;anchor=next}}; const coordinator=new SingleWriterCoordinator('AAECAwQFBgcICQoLDA0ODw','EBESExQVFhcYGRobHB0eHw','remote',transport,new GoogleSheetsSingleWriterProfileCodec(async(snapshot)=>({retired:false,verifiedEnvelopeIds:new Set(snapshot.rows.map(row=>row[0]??''))})),store); return {transport,coordinator,getDurable:()=>durable} }
