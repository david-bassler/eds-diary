...
it('verifies a legacy deletion against the migrated visible record id',async()=>{
  ...
  const checked=await db.__localDatabaseTesting.openDatabase()
  expect(checked.objectStoreNames.contains('painEntries')).toBe(false)
})
})
