...
export async function epochLocatorV2(diaryId:string,epochId:string): Promise<string> {
  return base64Url((await sha256(concatBytes(
    utf8('sync-v6/epoch-locator'), new Uint8Array([0]),
...
export function isAuthenticatedGoogleV2Transport(value:unknown):value is GoogleSheetsTransferableSingleWriterV2Transport{
  return typeof value==='object'&&value!==null&&AUTHENTICATED_TRANSPORTS.has(value as GoogleSheetsTransferableSingleWriterV2Transport)
}

/** Strict v2 Google wire adapter. It intentionally does not share the v1 profile identity or locator domains. */
export class GoogleSheetsTransferableSingleWriterV2Transport implements RemoteTransport {
  ...
  static async fromAuthenticatedSession(api:GoogleApiClient,diaryId:string,epochId:string):Promise<GoogleSheetsTransferableSingleWriterV2Transport>{
    ...
    const transport=new GoogleSheetsTransferableSingleWriterV2Transport(api,{...partial,googleAccountBinding})
  ...
  forEpoch(diaryId:string,epochId:string):Promise<GoogleSheetsTransferableSingleWriterV2Transport>{return GoogleSheetsTransferableSingleWriterV2Transport.fromAuthenticatedSession(this.api,diaryId,epochId)}
  ...
  private async verifyDrive(remoteId: string,preBound=false): Promise<void> {
    ...
    if ((!preBound||!empty)&&(keys.join('\0') !== 'app_format\0epoch_locator' || properties.app_format !== 'sync-v6' || properties.epoch_locator !== await expectedEpochLocator(this.binding))) throw new TransportError('integrity_failure', 'Protocol appProperties do not match the trusted epoch.')
