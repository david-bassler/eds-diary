...
export async function reconcileCreation(state: CreationState, transport: RemoteTransport, codec: TransportProfileCodec): Promise<CreationState> {
  ...
  for (const candidate of await transport.discover(state.locator)) {
    try { const snapshot = await transport.read(candidate.remoteId); codec.validate(snapshot); if (await codec.authenticateManifest(snapshot) === state.manifestFingerprint) authentic.push(candidate.remoteId) } catch { /* unauthenticated discovery candidates are ignored */ }
  }
