...
export class SingleWriterV1RemoteVerifier implements RemoteProfileVerifier {
  ...
  assertRecoveryBinding(binding: {diaryId:string;epochId:string;keyId:string;manifestFingerprint:string;recoveryGeneration:number;recoveryCommitment:string;accountBinding:string;anchor:RemoteAnchorV1|null}): void {
  ...
  async verify(snapshot: RemoteSnapshot): Promise<VerifiedRemoteState> {
    ...
    const verifiedEnvelopeIds=new Set(envelopes.map((envelope)=>envelope.envelopeId));return { profileId:SINGLE_WRITER_V1_PROFILE, profileState:null, snapshot, manifestFingerprint: fingerprint, retired: announcements.length === 1, verifiedEnvelopeIds, acceptedEnvelopeIds:new Set(verifiedEnvelopeIds), staleWriterEnvelopeIds:new Set<string>() }
  }
}
