...
async function persistRevision(db:IDBDatabase,store:LocalStoreName,value:Record<string,unknown>,fixedRevisionId?:string):Promise<void>{
  ...
  // Validation is deliberately before reservation. A locally generated row must
  // be acceptable to the same strict validator used at the remote boundary.
...
/** Persists the reconstructable outbox phase together with its generation. */
export async function transitionOutbox(envelopeId:string,status:'pending'|'remote_seen',expectedGeneration:number):Promise<number>{const db=await openDatabase(),loaded=await loadEpoch(db);return withDiaryLock(loaded.context.diaryId,async()=>{const current=await loadEpoch(db);if(current.state.operation_generation!==expectedGeneration)throw new Error('Stale network completion.');const read=db.transaction([STORES.outbox,STORES.envelopes],'readonly'),outboxRequest=read.objectStore(STORES.outbox).get(envelopeId),envelopeRequest=read.objectStore(STORES.envelopes).get(envelopeId),[stored,envelope]=await Promise.all([result<StoredOutbox|undefined>(outboxRequest),result<StoredEnvelope|undefined>(envelopeRequest)]);await complete(read);if(!envelope)throw new Error('Immutable envelope missing.');const item=stored??{id:envelopeId,epochId:current.context.epochId,envelopeId,rowBytes:envelope.rowBytes,status:'prepared'};const allowed=status==='pending'?item.status==='prepared'||item.status==='pending':item.status==='pending'||item.status==='remote_seen';if(!allowed)throw new Error('Invalid outbox transition.');if(item.status===status)return current.state.operation_generation;const state={...current.state,operation_generation:current.state.operation_generation+1},tag=await stateTag(current.rootKey,current.epochSalt,state),tx=db.transaction([STORES.outbox,STORES.state],'readwrite');tx.objectStore(STORES.outbox).put({...item,status});tx.objectStore(STORES.state).put({id:current.context.epochId,state,tag} satisfies StoredState);await complete(tx);return state.operation_generation})}

/** Derives pending work from immutable envelopes, never solely from flags. */
export async function pendingEnvelopes(remoteRows:ReadonlyArray<readonly string[]>):Promise<PreparedEnvelope[]>{const db=await openDatabase();await verifyLocalIntegrityFor(db);const loaded=await loadEpoch(db),tx=db.transaction(STORES.envelopes,'readonly'),items=await result<StoredEnvelope[]>(tx.objectStore(STORES.envelopes).index('byEpoch').getAll(loaded.context.epochId));await complete(tx);const remoteById=new Map<string,string>();for(const row of remoteRows){const bytes=decodeUtf8(canonicalBytes([...row]));const prior=remoteById.get(row[0]??'');if(prior!==undefined&&prior!==bytes)throw new Error('Envelope ID exists with different bytes.');remoteById.set(row[0]??'',bytes)}return items.filter(item=>{const remote=remoteById.get(item.envelopeId);if(remote!==undefined&&remote!==item.rowBytes)throw new Error('Envelope ID exists with different bytes.');return remote===undefined}).map(({envelopeId,iv,ciphertext,bytesHash})=>({envelopeId,iv,ciphertext,bytesHash}))}

/** The production coordinator adapter. Every method re-opens and authenticates
 * the active epoch; no in-memory flag is authoritative across a reload. */
export class IndexedDbCoordinatorStore implements CoordinatorStore {
  constructor(private readonly epochId:string){}
  async readAnchor():Promise<RemoteAnchor|null>{const loaded=await loadEpoch(await openDatabase());if(loaded.context.epochId!==this.epochId)throw new Error('Coordinator epoch is no longer active.');return loaded.state.remote_anchor}
  pending(remoteRows:ReadonlyArray<readonly string[]>):Promise<readonly PreparedEnvelope[]>{return pendingEnvelopes(remoteRows)}
  async generation():Promise<number>{const loaded=await loadEpoch(await openDatabase());if(loaded.context.epochId!==this.epochId)throw new Error('Coordinator epoch is no longer active.');return loaded.state.operation_generation}
  markPending(envelopeId:string,expectedGeneration:number):Promise<number>{return transitionOutbox(envelopeId,'pending',expectedGeneration)}
  markRemoteSeen(envelopeId:string,expectedGeneration:number):Promise<number>{return transitionOutbox(envelopeId,'remote_seen',expectedGeneration)}
  commitDurable(envelopeId:string,anchor:RemoteAnchor,expectedGeneration:number):Promise<void>{return commitDurableAck(this.epochId,envelopeId,expectedGeneration,anchor)}
}
