...
async function persistRevision(db:IDBDatabase,store:LocalStoreName,value:Record<string,unknown>,fixedRevisionId?:string):Promise<void>{
  ...
  const revisions=stored.sort((a,b)=>a.sequence-b.sequence).map(item=>item.revision),graph=validateRevisionGraph(revisions),heads=[...(graph.headsByRecord.get(recordId)??[])],profile=profileFor(store,id),revisionId=fixedRevisionId??base64Url(randomBytes(32))
