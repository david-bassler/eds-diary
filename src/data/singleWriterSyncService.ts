...
export class SingleWriterSyncService {
  ...
  static async createAuthenticated(session:SingleWriterProviderSession):Promise<SingleWriterSyncService>{
    ...
    const verifier=new FullRemoteVerifier({rootKey:active.rootKey,diaryId:active.diaryId,epochId:active.epochId,expectedManifestFingerprint:active.state.manifest_fingerprint,expectedKeyId:active.state.key_id,expectedRecoveryGeneration:active.state.recovery_generation,expectedRecoveryCommitment:active.state.recovery_urs_commitment,expectedGoogleAccountBinding:identityBinding,schemas:DOMAIN_SCHEMA_REGISTRY,oldAnchor:active.state.remote_anchor,...material})
    return SingleWriterSyncService.create(transport,session.codec(verifier),remote.remote_resource_id)
  }
