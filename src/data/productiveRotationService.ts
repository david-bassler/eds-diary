...
export class ProductiveRotationService {
  ...
  private dependencies(startFresh=false):RotationOrchestratorDependencies{
    ...
    return{persistence,
      ...
      createAndBootstrapRecovery:async raw=>{const state=raw as ConcreteRotationState,context=await this.context(state),root=await this.repository.successorRoot(state.newEpochId,state.newWrapId),remoteId=await this.remoteId(state),wire=await this.successorWire(state),snapshot=await wire.read(remoteId),anchor=await createAnchorV1(context.diaryId,state.newEpochId,snapshot.rows),account=await this.session.remoteIdentityBinding(wire),stored=await this.repository.artifact<RecoveryArtifact>(`${state.rotationId}:recovery`),artifact=stored??await createRecovery({diary_id:context.diaryId,epoch_id:state.newEpochId,key_id:state.newKeyId,RK_epoch:base64Url(root),manifest_fingerprint:context.manifestFingerprint,remote_anchor:anchor,google_account_binding:account,recovery_generation:state.recoveryGeneration??this.targetRecoveryGeneration(this.source??await this.repository.verifiedActiveEpoch()),created_at:this.now()},this.urs),candidate=await recoverRootKeyCandidate(artifact,this.urs),authority=await this.session.recoveryAuthority(wire,state.creationLocator,remoteId),verifier=new RecoveryBootstrapVerifier({authority,schemas:DOMAIN_SCHEMA_REGISTRY});await activateRecoveredRoot(candidate,verifier,(verifiedCandidate,bootstrap)=>persistRecoveredProfile(verifiedCandidate,bootstrap,DOMAIN_SCHEMA_REGISTRY,{databaseName:`eds-diary-recovery-gate-${state.rotationId}`,cleanupAfterVerify:true}));if(!stored)await this.repository.putArtifact(`${state.rotationId}:recovery`,artifact);if(state.migrationKind==='remote_enablement')await this.session.publishRecoveryArtifact(this.urs,artifact);await this.hit('after-recovery-verified');return artifact.recovery_artifact_id},
