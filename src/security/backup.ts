...
export async function createBackup(context:BackupContext,id=randomBytes(32),iv=randomBytes(12)):Promise<SyncBackupV5>{
  const backup_id=base64Url(id),remote=context.remoteRows,remoteSet=new Set(remote.map((row)=>JSON.stringify(row))),remoteById=new Map<string,string>();for(const row of remote){const encoded=JSON.stringify(row),prior=remoteById.get(row[0]);if(prior!==undefined&&prior!==encoded)throw new Error('Duplicate envelope_id has different bytes.');remoteById.set(row[0],encoded)}const localRows=context.localEnvelopes.map((e)=>[e.envelopeId,e.iv,e.ciphertext]as const);for(const row of localRows){const remoteBytes=remoteById.get(row[0]);if(remoteBytes!==undefined&&remoteBytes!==JSON.stringify(row))throw new Error('Duplicate envelope_id has different bytes.')}const pending=localRows.filter((row)=>!remoteSet.has(JSON.stringify(row))),union=new Set([...remote,...pending].map((row)=>JSON.stringify(row)))
