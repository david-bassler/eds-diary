...
export interface EpochMigrationData {migration_id:string;migration_kind:'normal'|'local_rotation'|'remote_enablement'|'recovery_rekey'|'emergency';source:{source_epoch_id:string;source_manifest_fingerprint:string;source_anchor:unknown;source_lineage_snapshot_hash:string;source_semantic_snapshot_hash:string};result_semantic_snapshot_hash:string;active_head_count:number;tombstone_head_count:number}
export function createEpochMigrationData(data:EpochMigrationData):EpochMigrationData{if((data.migration_kind==='normal'||data.migration_kind==='remote_enablement'||data.migration_kind==='local_rotation'||data.migration_kind==='recovery_rekey')&&data.result_semantic_snapshot_hash!==data.source.source_semantic_snapshot_hash)throw new Error('Unchanged migration cannot change semantics.');if(!Number.isSafeInteger(data.active_head_count)||data.active_head_count<0||!Number.isSafeInteger(data.tombstone_head_count)||data.tombstone_head_count<0)throw new Error('Invalid migration head counts.');return structuredClone(data)}
