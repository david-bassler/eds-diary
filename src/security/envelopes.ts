...
import { validateRevisionV1, type RevisionV1 } from './revisions'
...
export interface EnvelopeReservation { reserve(envelopeId:string):Promise<void>; verifyReservation?(envelopeId:string):Promise<void>; afterReservation?(envelopeId:string):Promise<void>; afterEncryption?(envelope:PreparedEnvelope):Promise<void>; persist(envelope:PreparedEnvelope):Promise<void> }
...
export async function prepareEnvelopeV1(rootKey:Uint8Array,epochSalt:Uint8Array,context:EnvelopeContext,revision:RevisionV1,journal:EnvelopeReservation,iv=randomBytes(12),idBytes=randomBytes(32)):Promise<PreparedEnvelope>{
  fixedBase64Url(context.diaryId,16,'diary_id');fixedBase64Url(context.epochId,16,'epoch_id');validateRevisionV1(revision)
  const envelopeId=base64Url(idBytes);await journal.reserve(envelopeId);await journal.verifyReservation?.(envelopeId);await journal.afterReservation?.(envelopeId)
  ...
  const encodedIv=base64Url(encrypted.iv),ciphertext=base64Url(encrypted.ciphertext);const envelope={envelopeId,iv:encodedIv,ciphertext,bytesHash:base64Url(await sha256(canonicalBytes([envelopeId,encodedIv,ciphertext])))};await journal.afterEncryption?.(envelope);await journal.persist(envelope);return envelope
}
export async function openEnvelopeV1(rootKey:Uint8Array,epochSalt:Uint8Array,context:EnvelopeContext,envelope:PreparedEnvelope):Promise<RevisionV1>{
  ...
  const key=await deriveEnvelopeKey(rootKey,epochSalt,id),frame=await aesGcmDecrypt(key,ciphertext,recordAad(context,envelope.envelopeId,bucket),iv),revision=parseCanonicalJson(unframe(frame)) as unknown as RevisionV1;validateRevisionV1(revision);return revision
}
...
/** Backwards-compatible aliases for the existing v1 envelope format. */
export const prepareEnvelope = prepareEnvelopeV1
export const openEnvelope = openEnvelopeV1
