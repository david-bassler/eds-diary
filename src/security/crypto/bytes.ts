...
export function uint32be(value: number): Uint8Array {
  if (!Number.isSafeInteger(value) || value < 0 || value > 0xffffffff) throw new Error('Value is outside uint32.')
...
export function fixedBase64Url(value: string, length: number, field = 'value'): Uint8Array {
  const decoded = fromBase64Url(value)
  if (decoded.byteLength !== length) throw new Error(`${field} must decode to ${length} bytes.`)
  return decoded
}
