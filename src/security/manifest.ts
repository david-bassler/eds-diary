...
export const SINGLE_WRITER_V1_SCHEMA_ALLOWLIST = ['activity-entry/v1','activity-type-settings/v1','epoch-migration-sw-v1','medication-entry/v1','medication-prescription/v1','pain-entry/v1','pain-type-settings/v1','rotation-announcement-sw-v1'] as const
...
export const SCHEMA_ALLOWLIST = SINGLE_WRITER_V1_SCHEMA_ALLOWLIST
export type ProtectedManifest = ProtectedManifestV1
export async function schemaRegistryHash(schemas:Readonly<Record<string,unknown>>):Promise<string>{const names=Object.keys(schemas).sort();if(names.join('\0')!==[...SINGLE_WRITER_V1_SCHEMA_ALLOWLIST].sort().join('\0'))throw new Error('Schema registry does not match the Single-Writer allowlist.');const entries=[];for(const record_schema of names)entries.push({record_schema,schema_sha256:base64Url(await sha256(canonicalBytes(schemas[record_schema] as never)))});return base64Url(await sha256(canonicalBytes(entries)))}
export async function prepareManifest(rootKey:Uint8Array,epochSalt:Uint8Array,context:ManifestContext,payload:ProtectedManifestV1,iv=randomBytes(12)):Promise<ManifestCells>{fixedBase64Url(context.diaryId,16);fixedBase64Url(context.epochId,16);validateManifest(payload,context);const key=await deriveManifestKey(rootKey,epochSalt),encrypted=await aesGcmEncrypt(key,canonicalBytes(payload as never),aad(context),iv);return{format:'sync-v5',version:'5',manifestIv:base64Url(encrypted.iv),manifestCiphertext:base64Url(encrypted.ciphertext)}}
export async function openManifest(rootKey:Uint8Array,epochSalt:Uint8Array,context:ManifestContext,cells:ManifestCells):Promise<ProtectedManifestV1>{if(cells.format!=='sync-v5'||cells.version!=='5')throw new Error('Invalid manifest header.');const key=await deriveManifestKey(rootKey,epochSalt),plain=await aesGcmDecrypt(key,fromBase64Url(cells.manifestCiphertext),aad(context),fixedBase64Url(cells.manifestIv,12));const payload=parseCanonicalJson(plain) as unknown as ProtectedManifestV1;validateManifest(payload,context);return payload}
...
function validateManifest(p:ProtectedManifestV1,c:ManifestContext):void{
  ...
  if(!Array.isArray(p.record_schema_allowlist)||p.record_schema_allowlist.join('\0')!==SINGLE_WRITER_V1_SCHEMA_ALLOWLIST.join('\0'))throw new Error('Manifest allowlist mismatch.')
