...
export interface ShoulderDetailSelectorProps {
  side: ShoulderSide
...
function createHitMapData(imageData: ImageData): HitMapData {
  ...
  const colorLookup = new Map(
    SHOULDER_REGIONS.map((region, index) => [
...
export function ShoulderDetailSelector({
  side,
  value,
  onChange,
  onClose,
}: ShoulderDetailSelectorProps) {
  ...
  const sideLabel = side === 'left' ? 'Linke Schulter' : 'Rechte Schulter'
  ...
  useEffect(() => {
    ...
    SHOULDER_REGIONS.forEach((region, index) => {
  ...
  function selectAtPointer(event: ReactPointerEvent<HTMLCanvasElement>): void {
    ...
    const region = SHOULDER_REGIONS[regionIndex]
  ...
  return (
    <section
      className="shoulder-detail-selector"
      aria-label={`${sideLabel} genauer auswählen`}
      data-side={side}
    >
      <div className="shoulder-detail-selector__heading">
        <div>
      ...
      <div
        className="shoulder-detail-selector__artwork"
        data-mirrored={mirrored}
        role="img"
        aria-label={`${sideLabel}: Bereich antippen`}
      >
        <img
          className="shoulder-detail-selector__image"
          src={SHOULDER_IMAGE}
          alt=""
          aria-hidden="true"
          draggable={false}
        ...
        <canvas
          ref={overlayRef}
          className="shoulder-detail-selector__overlay"
      ...
      <div className="shoulder-detail-selector__selection" aria-live="polite">
        {value.length === 0 ? (
          <span>Noch keine Feinauswahl</span>
        ) : (
          <>
            <span>{value.map(shoulderDetailLabel).join(', ')}</span>
            <button type="button" onClick={() => onChange([])}>Feinauswahl löschen</button>
          </>
        )}
      </div>

      <details className="shoulder-detail-selector__list">
        <summary>Bereiche alternativ als Liste auswählen</summary>
        <div className="shoulder-detail-selector__list-grid">
          {SHOULDER_REGIONS.map((region) => (
            <label key={region.id}>
              <input
                type="checkbox"
