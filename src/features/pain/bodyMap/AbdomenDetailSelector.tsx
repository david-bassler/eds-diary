...
export interface AbdomenDetailSelectorProps {
...
const ABDOMEN_REGIONS: readonly RegionDefinition[] = [
...
const ABDOMEN_IMAGE = `${import.meta.env.BASE_URL}body-map/details/abdomen-hitmap.png?v=1`
...
function createHitMapData(imageData: ImageData): HitMapData {
  ...
  const colorLookup = new Map(
    ABDOMEN_REGIONS.map((region, index) => [
...
export function AbdomenDetailSelector({
  value,
  onChange,
  onClose,
}: AbdomenDetailSelectorProps) {
  ...
  useEffect(() => {
    ...
    ABDOMEN_REGIONS.forEach((region, index) => {
  ...
  function selectAtPointer(event: ReactPointerEvent<HTMLCanvasElement>): void {
    ...
    const region = ABDOMEN_REGIONS[regionIndex]
  ...
  return (
    <section
      className="abdomen-detail-selector"
      aria-label="Bauch genauer auswählen"
    >
      <div className="abdomen-detail-selector__heading">
        <div>
          <strong>Bauch</strong>
      ...
      <div
        className="abdomen-detail-selector__artwork"
        role="img"
        aria-label="Bauch: Bereich antippen"
      >
        <img
          className="abdomen-detail-selector__image"
          src={ABDOMEN_IMAGE}
          alt=""
          aria-hidden="true"
          draggable={false}
        ...
        <canvas
          ref={overlayRef}
          className="abdomen-detail-selector__overlay"
      ...
      <div className="abdomen-detail-selector__selection" aria-live="polite">
        {value.length === 0 ? (
          <span>Noch keine Feinauswahl</span>
        ) : (
          <>
            <span>{value.map(abdomenDetailLabel).join(', ')}</span>
      ...
      <details className="abdomen-detail-selector__list">
        <summary>Bereiche alternativ als Liste auswählen</summary>
        <div className="abdomen-detail-selector__list-grid">
          {ABDOMEN_REGIONS.map((region) => (
            <label key={region.id}>
              <input
                type="checkbox"
